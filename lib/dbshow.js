...
dbshow.showport = function(opp_json,porttotal,result){
  ...
  for(var i = 0;i<porttotal;i++){
    var p = result[i];
    var opp_usingport = opp_json[result[i].sharenum][0];
    var opp_bridgeport = opp_json[result[i].sharenum][1];
    var opp_disableport = opp_json[result[i].sharenum][2];
    ...
    }else if(p.porttype === 'FDF'){
      contents += dbshow.FDF(opp_usingport,opp_bridgeport,opp_disableport,p._id,p.sharenum,p.portname,p.nodename,p.oppnodename,p.portnumber,p.porttype,p.cabletype,p.usingport,p.bridgeport,p.disableport,p.availableport,p.reference,p.usage,p.connectstat,p.latestupdate)
    }else{
...
dbshow.showportOne = function(opp_usingport,opp_bridgeport,opp_disableport,result){
  ...
  if(p.porttype === "CDF_E1" || p.porttype === 'CDF'){
    ...
    content = dbshow.FDF(opp_usingport,opp_bridgeport,opp_disableport,p._id,p.sharenum,p.portname,p.nodename,p.oppnodename,p.portnumber,p.porttype,p.cabletype,p.usingport,p.bridgeport,p.disableport,p.availableport,p.reference,p.usage,p.connectstat,p.latestupdate)
  }else{
...
dbshow.FDF = function(opp_usingport,opp_bridgeport,opp_disableport,_id,sharenum,portname,nodename,oppnodename,portnumber,porttype,cabletype,usingport,bridgeport,disableport,availableport,reference,usage,connectstat,latestupdate){
    var cablecolor = 'gray';
    ...
    //포트 넓이 높이 결정
    var port_height = 9.3 + 2.2*Math.ceil(portnumber/12);
    if(portnumber<8){
      var port_width = 13;
      var pip_width = 1.7*portnumber;
    }else if(portnumber >= 8 && portnumber < 12){
      var port_width = 1.7*portnumber;
      var pip_width = 1.7*portnumber;
  ...
  ${parseInt(j)+1}. ${connectstat[i][j].oppnodename}(으)로 가는${connectstat[i][j].portname}의 ${connectstat[i][j].num}번 포트`
      ...
      plus_ports += `
  <span id="${id}" class="num n${i}" title="${title}" data-id = '{"_id" : ${_id}, "sharenum" : ${sharenum}, "num" : ${i}, "nodename" : "${nodename}","oppnodename" : "${oppnodename}", "portname" : "${portname}"}' style="background:${background_color}">${i}</span>
  <input type="text" value="${usage[i]}" title="${u_title}" class="usage u${i}" id="${i}" style="display:none;" data-id='{"_id" : ${_id}, "num" : ${i}}'>
  <input type="text" value="${reference[i]}" title="${r_title}" class="ref r${i}" id="${i}" style="display:none;" data-id='{"_id" : ${_id}, "num" : ${i}}'>`
    }
    return `
  <div class="FDF" id="p${sharenum}" style="height:${port_height}rem; width:${port_width}rem">
  <div id="disable_party_none" style="display: none">
  <form action="/delete/port" method="post" class="deleting_port" onsubmit="return check_delete_port(${sharenum})">
  ...
  <span class="porttype">${porttype}</span>
  <input type="button" class="spread" value="펼쳐보기" data-id='{"sharenum":${sharenum},"portnumber":${portnumber},"port_width":${port_width}}' onclick="spread_FDF(this)">
  <span class="h port">포트</span>
  <span class="h use">용도</span>
  <span class="h refer">비고</span>
  </div>
  <div class="portsinport" style="width:${pip_width}rem">
  ${plus_ports}
  </div>
  </div>`
}
...
${parseInt(j)+1}. ${connectstat[i][j].oppnodename}(으)로 가는${connectstat[i][j].portname}의 ${connectstat[i][j].num}번 포트`
    ...
    if(portnumber>32 && portnumber <65){
      ...
      plus_ports += `
<div class="CDF_E1_row"><span id="${id}" class="num n${i}" title="${title}" data-id = '{"_id" : ${_id}, "sharenum" : ${sharenum}, "num" : ${i}, "nodename" : "${nodename}", "oppnodename" : "${oppnodename}", "portname" : "${portname}"}' style="background:${background_color}">${i}</span>
<input type="text" value="${usage[i]}" title="${u_title}" class="usage u${i}" style="display:none;" data-id='{"_id" : ${_id}, "num" : ${i}}'>
<input type="text" class="ref r${i}" title="${r_title}" style="display:none;" value="${reference[i]}" data-id='{"_id" : ${_id}, "num" : ${i}}'></div>`
